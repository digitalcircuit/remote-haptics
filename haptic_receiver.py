...
async def main(config_file, write_config, listen_addr, disable_ssl, ssl_cert, ssl_key):
    ...
    tasks.append(asyncio.create_task(api.start_server(listen_addr, disable_ssl, ssl_cert, ssl_key, haptics_receiver.on_session_new_cb, haptics_receiver.on_session_end_cb, haptics_receiver.on_session_type_set_cb, haptics_receiver.on_haptics_updated_cb)))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", "--config-file", help="path to configuration file (default: config/config-receiver.ini)", metavar="<path>", default=os.path.join("config", "config-receiver.ini"))
    parser.add_argument("-w", "--write-config", help="write out a sample configuration file then exit", action="store_true")
    parser.add_argument("-l", "--listen", help="address to listen on (default: 127.0.0.1:{0})".format(haptics.NET_DEFAULT_PORT), metavar="<hostname:port>", default = "127.0.0.1:{0}".format(haptics.NET_DEFAULT_PORT))
    ...
    parser.add_argument("--ssl-key", help="SSL/TLS private key", metavar="<server.key>", default = os.path.join("certs", "server.key"))
    args = parser.parse_args()
    if not ":" in args.listen:
        print("Listen address must include ':', e.g. '127.0.0.1:{0}'".format(haptics.NET_DEFAULT_PORT))
        raise SystemExit

    asyncio.run(main(args.config_file, args.write_config, args.listen, args.insecure, args.ssl_cert, args.ssl_key))
