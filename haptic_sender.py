...
async def main(config_file, write_config, server_addr, disable_ssl, ssl_cert):
    ...
    tasks.append(asyncio.create_task(api.start_client(server_addr, disable_ssl, ssl_cert, haptics_sender.on_haptics_request_cb, api.SessionType.LIVE)))
...
if __name__ == "__main__":
    ...
    parser.add_argument("server_addr", help="RemoteHaptics API server address (default port: {0})".format(haptics.NET_DEFAULT_PORT), metavar="<hostname:port>", nargs="?")
    parser.add_argument("-c", "--config-file", help="path to configuration file (default: config/config-sender.ini)", metavar="<path>", default=os.path.join("config", "config-sender.ini"))
    ...
    asyncio.run(main(args.config_file, args.write_config, args.server_addr, args.insecure, args.ssl_cert))
