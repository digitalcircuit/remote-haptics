...
class FeedbackMapper:
    ...
    def apply_outputs(self, strong_intensity, weak_intensity):
        ...
        strong_magnitude = int(
            max(
                0,
                min(
                    evdev_driver.RumbleDevice.RUMBLE_MAX * strong_intensity,
                    evdev_driver.RumbleDevice.RUMBLE_MAX,
                ),
            )
        )
        weak_magnitude = int(
            max(
                0,
                min(
                    evdev_driver.RumbleDevice.RUMBLE_MAX * weak_intensity,
                    evdev_driver.RumbleDevice.RUMBLE_MAX,
                ),
            )
        )
        for rumble_device in self.__outputs:
            rumble_device.rumble(
                duration_ms=haptics.PERSIST_DURATION_SECS * 1000,
    ...
    def parse_inputs(self, haptics_inputs):
        ...
        if matches < 1:
            self.__logger.error(
                "No matching haptics inputs found!  Haptics input count: {0}, requested IDs/indices: {1} (override/full: {2})".format(
                    len(haptics_inputs), self.__input_ids, self.__input_ids_override
                )
            )
            self.__physics_interrupt()
            return
    ...
    def __physics_update(self, value):
        ...
        if self.__previous_time:
            time_delta = (
                datetime.datetime.now(datetime.timezone.utc) - self.__previous_time
            ).total_seconds()
        ...
        self.__physics_calc_avg_value = (value * avg_percent_old) + (
            self.__physics_calc_avg_value * (1 - avg_percent_old)
        )
        ...
        if time_delta <= haptics.PERSIST_DURATION_SECS:
            ...
            if self.__physics_calc_impact > 250:
                ...
                self.__physics_calc_impact_boost = min(
                    7, self.__physics_calc_impact * 0.2 * time_ratio
                )
            else:
                # Decrement impact boost on weaker numbers
                self.__physics_calc_impact_boost = max(
                    0, self.__physics_calc_impact_boost - (time_ratio / 2)
                )
            ...
            if haptics.VERBOSE_RECEIVER_PHYSICS:
                self.__logger.debug(
                    "More than {0}s elapsed ({1}s), ignoring physics".format(
                        haptics.PERSIST_DURATION_SECS, time_delta
                    )
                )
        ...
        if haptics.VERBOSE_RECEIVER_PHYSICS:
            print(
                "Physics {0} = val: {1: >5.4f}, delta: {2: >9.4f}, avg: {3: >9.4f}, avg_delta: {4: >9.4f}, boost: {5: >6.2f}, impact: {6: >11.4f}, weak: {7: >5.4f}, strong: {8: >5.4f}".format(
                    self.__input_ids,
                    value,
                    value_delta,
                    self.__physics_calc_avg_value,
                    avg_value_delta,
                    self.__physics_calc_impact_boost,
                    self.__physics_calc_impact,
                    output_weak,
                    output_strong,
                ),
                flush=True,
        ...
        if (
            self.__physics_calc_impact > 0
            or self.__physics_calc_impact_boost > 0
            or avg_value_delta > 0
        ):
            # Not finished processing, schedule another update
            if not self.__physics_timer:
                self.__physics_timer = asyncio.get_event_loop().call_later(
                    haptics.MAX_UPDATE_RATE_SECS, self.__physics_timer_cb
                )
...
class ReceiverManager:
    ...
    def recording_start(self, peername):
        ...
        peerhost = peername[0]
        peerhost_safe = "".join(
            c for c in peerhost if c.isalnum() or c in keepcharacters
        ).rstrip()
    ...
    def write_sample_config(self):
        ...
        config.set(
            self.Config.DEVICES,
            "# specify +number (e.g. +4) to exclude that index from averaging (1.0 is treated as full 1.0)",
        )
        ...
        for rumble_device in rumble_devices:
            ...
            config_devices[
                self.Config.OPT_INITIAL_NAME_FORMAT.format(filtered_device_id)
            ] = rumble_device.name
    ...
    def load_config(self):
        ...
        if not "recording" in config:
            self.__logger.warning(
                "Configuration file '{0}' doesn't specify recording mode, write a new one with '--write'?".format(
                    self.__config_file_path
                )
            )
            return False
        config_recording = config[self.Config.RECORDING]
        self.__recording_enabled = config_recording.getboolean(
            self.Config.RECORDING_OPT_ENABLED
        )
        self.__recording_dir = config_recording[self.Config.RECORDING_OPT_DEST_DIR]
        if self.__recording_enabled:
            self.__logger.debug(
                "Recording live sessions enabled, saving to '{0}'".format(
                    self.__recording_dir
                )
            )
        else:
        ...
        if not "devices" in config:
            self.__logger.warning(
                "Configuration file '{0}' doesn't specify any devices, write a new one with '--write'?".format(
                    self.__config_file_path
                )
            )
            return False
        ...
        for rumble_device in rumble_devices:
            ...
            if filtered_device_id in config_devices:
                ...
                self.__logger.debug(
                    "Mapping device '{0}' to haptics indices '{1}' (overrides: '{2}')".format(
                        filtered_device_id, haptics_ids_normal, haptics_ids_override
                    )
                )
                self.__feedback_mappers.append(
                    FeedbackMapper(
                        haptics_ids_normal, haptics_ids_override, [rumble_device]
                    )
                )
            else:
                self.__logger.debug(
                    "Ignoring unconfigured device '{0}' ({1})".format(
                        filtered_device_id, rumble_device.name
                    )
                )

        if not len(self.__feedback_mappers):
            self.__logger.warning(
                "No rumble devices configured, check configuration file '{0}' or write a new one with '--write'".format(
                    self.__config_file_path
                )
            )
