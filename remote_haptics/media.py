...
class MediaPlayer():
    LIKELY_SOCKET_ERRORS = (BrokenPipeError, ConnectionResetError, OSError, TimeoutError)

    def __init__(self, media_base_dir, external_paused_set_cb, external_position_set_cb, external_quit_cb):
    ...
    def paused(self):
        ...
        try:
            ...
            if haptics.VERBOSE_MEDIA:
                self.__logger.debug("[{0}] Error when fetching paused state: {1}".format(self.__logging_id, ex))
            else:
                self.__logger.debug("[{0}] Error when fetching paused state: {1}".format(self.__logging_id, type(ex).__name__))
            return None
    ...
    def paused(self, value):
        ...
        if haptics.VERBOSE_MEDIA:
            self.__logger.debug("[{0}] Setting paused = {1}".format(self.__logging_id, value))
    ...
    def position(self):
        ...
        try:
            ...
            if not self.__media_file:
                ...
                return None
            if haptics.VERBOSE_MEDIA:
                self.__logger.debug("[{0}] Error when fetching position: {1}".format(self.__logging_id, ex))
            else:
                self.__logger.debug("[{0}] Error when fetching position: {1}".format(self.__logging_id, type(ex).__name__))
            return None
    ...
    def position(self, value):
        ...
        if value > (cur_position - haptics.MEDIA_POSITION_SKEW_SECS) and value < (cur_position + haptics.MEDIA_POSITION_SKEW_SECS):
            #if haptics.VERBOSE_MEDIA:
        ...
        if haptics.VERBOSE_MEDIA:
            ...
            self.__logger.debug("[{0}] Delta of {1:f}s, seeking to {2:f}s from {3:f}s".format(self.__logging_id, deviation, round(value, haptics.TIME_PRECISION_PLACES), round(cur_position, haptics.TIME_PRECISION_PLACES)))

        if self.__media_ended:
            if haptics.VERBOSE_MEDIA:
                self.__logger.debug("[{0}] Media already ended, restarting player to seek to {1:f}s".format(self.__logging_id, round(value, haptics.TIME_PRECISION_PLACES)))
            self.__start_player()
        self.__ignore_nearby_events()
    ...
    def play_with_offset(self, time_delta, offset, media_file):
        ...
        if time_delta + offset < 0 and self.__get_media_hash() not in self.__media_negative_offset_warn_hash:
            self.__logger.debug("[{0}] Media start time set for the future (loaded early?), will pause for {1}s to catch up".format(self.__logging_id, -(time_delta + offset)))
        ...
        if success:
            if haptics.VERBOSE_MEDIA:
                self.__logger.debug("[{0}] Playing media '{1}' with offset {2}s (at time delta {3}s)".format(self.__logging_id, os.path.basename(media_file), offset, time_delta))
    ...
    def quit(self, force_shutdown):
        if self.__player:
            ...
            if force_shutdown:
                if haptics.VERBOSE_MEDIA:
                    self.__logger.debug("[{0}] Forcibly shutting down media player".format(self.__logging_id))
                self.__player.terminate()
            else:
                if haptics.VERBOSE_MEDIA:
                    self.__logger.debug("[{0}] Normally shutting down media player".format(self.__logging_id))
                self.__player.quit()

    def __ignore_nearby_events(self):
        self.__ignore_event_until = datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(seconds=0.1)
    ...
    def __ensure_player(self):
        ...
        # Attach handlers
        self.__ignore_nearby_events()
        self.__player.bind_event("start-file", self.__media_event_ignore_other_events_cb)
        self.__player.bind_event("file-loaded", self.__media_event_ignore_other_events_cb)
    ...
    def __start_player(self):
        self.__ensure_player()
        if not self.__player:
            self.__logger.warning("[{0}] Can't play media '{1}', unable to start Python MPV over IPC".format(self.__logging_id, os.path.basename(self.__media_file)))
            return False
    ...
    def __media_toggle_pause_cb(self, name, value):
        ...
        if self.__player_external_paused_cb:
            self.__async_main_loop.call_soon_threadsafe(self.__player_external_paused_cb, value)
    ...
    def __media_seek_cb(self, name, value):
        ...
        if haptics.VERBOSE_MEDIA:
            print("[{0}] Media seek: {1} (with offset: {2})".format(self.__logging_id, self.position, self.position_with_offset))

        # Set seek position
        if self.__player_external_position_set_cb:
            # Media player runs on a different thread
            self.__async_main_loop.call_soon_threadsafe(self.__player_external_position_set_cb, self.position_with_offset)
