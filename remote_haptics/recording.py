...
RECORDING_DUPLICATE_INTERVAL_SECS = max(0, RECORDING_DUPLICATE_INTERVAL_SECS - haptics.PERSIST_DURATION_SECS)
...
class EntryAbstract(ABC):
    ...
    def with_offset(self, offset):
        return EntryAbstract(self.session_start + datetime.timedelta(seconds=offset), self.time_delta + offset)
...
class EntryInputs(EntryAbstract):
    ...
    def to_record_str(self) -> str:
        return "{0}:{1}\n".format(super(EntryInputs, self).to_record_str(), ",".join(map(str, self.inputs)))

    def with_offset(self, offset):
        return EntryInputs(self.session_start + datetime.timedelta(seconds=offset), self.time_delta + offset, self.inputs)
...
class EntryRemark(EntryAbstract):
    ...
    def to_record_str(self) -> str:
        return "{0}:{1}{2}\n".format(super(EntryRemark, self).to_record_str(), Recording.LINE_REMARK, self.remark)

    def with_offset(self, offset):
        return EntryRemark(self.session_start + datetime.timedelta(seconds=offset), self.time_delta + offset, self.remark)
...
class EntryMediaAbstract(EntryAbstract):
    ...
    def to_record_str(self) -> str:
        ...
        return "{0}:{1}{2}".format(super(EntryMediaAbstract, self).to_record_str(), Recording.LINE_MEDIA, id_marker)

    def with_offset(self, offset):
        return EntryMediaAbstract(self.session_start + datetime.timedelta(seconds=offset), self.time_delta + offset, self.id)
...
class EntryMediaStop(EntryMediaAbstract):
    ...
    def to_record_str(self) -> str:
        return "{0}:{1}\n".format(super(EntryMediaStop, self).to_record_str(), Recording.MEDIA_CMD_STOP)

    def with_offset(self, offset):
        return EntryMediaStop(self.session_start + datetime.timedelta(seconds=offset), self.time_delta + offset, self.id)
...
class EntryMediaPlay(EntryMediaAbstract):
    ...
    def to_record_str(self) -> str:
        return "{0}:{1}:{2}\n".format(super(EntryMediaStop, self).to_record_str(), offset, media_file)

    def with_offset(self, offset):
        return EntryMediaPlay(self.session_start + datetime.timedelta(seconds=offset), self.time_delta + offset, self.id, self.offset, self.media_file)
...
def get_entry_from_line(recording_start, line):
    ...
    if line_content.startswith(Recording.LINE_REMARK):
        ...
        return EntryRemark(recording_start, time_delta, line_content[len(Recording.LINE_REMARK):])
        ...
        if media_data[0] == Recording.MEDIA_CMD_STOP:
            ...
            return EntryMediaPlay(recording_start, time_delta, id, media_offset, media_filename)
    else:
        # Haptics values
        # > 13.335039:0.301961,1
        return EntryInputs(recording_start, time_delta, [float(i) for i in line_content.split(",")])
...
class HapticsWriter():
    def __init__(self, filename):
        ...
        self.__file_writer.write(Recording.HEADER_TIMESTAMP + time_now.isoformat(sep=" ", timespec=haptics.TIME_PRECISION_ISO) + "\n")
    ...
    def append_record(self, haptics_entries):
        ...
        if haptics_entries != self.__last_haptics_entries or (time_now - self.__last_haptics_entries_duped).total_seconds() > RECORDING_DUPLICATE_INTERVAL_SECS:
            # Not duplicate or time elapsed, save

            # Insert UTC timestamp comment if not recently done
            if (time_now - self.__last_timestamp_comment).total_seconds() > RECORDING_TIMESTAMP_INTERVAL_SECS:
                self.__file_writer.write("# timestamp = {0}\n".format(time_now.isoformat(sep=" ", timespec=haptics.TIME_PRECISION_ISO)))
                self.__last_timestamp_comment = time_now

            # Save actual haptics data
            self.__file_writer.write(EntryInputs(self.__session_start, time_delta, haptics_entries).to_record_str())
    ...
    def __flush_file_timer_cb(self):
        if not self.__file_writer.closed:
            ...
            self.__flush_timer = asyncio.get_event_loop().call_later(RECORDING_FLUSH_INTERVAL_SECS, self.__flush_file_timer_cb)

    def close(self):
        # Record end time, too
        time_now = datetime.datetime.now(datetime.timezone.utc)
        self.__file_writer.write(Recording.FOOTER_TIMESTAMP + time_now.isoformat(sep=" ", timespec=haptics.TIME_PRECISION_ISO) + "\n")
...
class HapticsReader():
    ...
    def __restart(self):
        ...
        if first_line.strip() != Recording.HEADER_VERSION:
            raise Exception("Invalid recording file, header version is not the first line!  First line: {}".format(first_line))
        ...
        if not timestamp_line.startswith(Recording.HEADER_TIMESTAMP):
            raise Exception("Invalid recording file, header timestamp is not the second line!  Second line: {}".format(timestamp_line))
    ...
    def get_records_until(self, time_delta, max_prev_seconds = 3):
        ...
        while not self.__reached_end and (not self.__current_record or self.__current_record.time_delta < time_delta):
            # Include previous record if existing
            if self.__current_record:
                records.append(self.__current_record)

            while len(records) and records[0].time_delta < (time_delta - max_prev_seconds):
...
def merge_recordings(output_filename, recording_filenames):
    ...
    for index in range(0, len(readers)):
        offsets[index] = (readers[index].recording_start - earliest_recording_start).total_seconds()

    # Start output file
    file_writer = open(output_filename, mode='w', encoding = Recording.ENCODING)
    file_writer.write(Recording.HEADER_VERSION + "\n")
    file_writer.write(Recording.HEADER_TIMESTAMP + earliest_recording_start.isoformat(sep=" ", timespec=haptics.TIME_PRECISION_ISO) + "\n")
    ...
    while True:
        for index in range(0, len(readers)):
            if not reader_entries[index]:
                ...
                if reader_entries[index]:
                    reader_entries[index] = reader_entries[index].with_offset(offsets[index])
        ...
        for index in range(0, len(readers)):
            ...
            if oldest_delta is None or (reader_entries[index].time_delta < oldest_delta):
    ...
    last_recording_end = earliest_recording_start + datetime.timedelta(seconds=ending_time_delta)
    file_writer.write(Recording.FOOTER_TIMESTAMP + last_recording_end.isoformat(sep=" ", timespec=haptics.TIME_PRECISION_ISO) + "\n")
    file_writer.close()
