...
class PlayerUI():
    ...
    def on_remark_processed_cb(self, remark):
        if not self.__hide_remarks:
            ...
            print("[{0}] {1}".format(remark.to_datetime().astimezone(None).strftime("%I:%M:%S %p %Z"), remark.remark))
    ...
    def __print_help(self):
        ...
        self.__print_notice("pos:                   show current playback status/position")
        self.__print_notice("info:                  show recording information")
        self.__print_notice("hide:                  don't show remarks from the recording file")
        self.__print_notice("show:                  show remarks from the recording file (default)")
    ...
    def __print_help_timestamp_examples(self):
        ...
        example_time = self.__haptics_player.recording_start + datetime.timedelta(seconds=5.25)
        # 2022-05-27 23:28
        self.__print_notice(" Example 1 (local):  go {0}".format(example_time.astimezone(None).replace(tzinfo=None).isoformat(sep=" ", timespec="seconds")))
        # 2022-05-27 23:28:57.104000-04:00
        self.__print_notice(" Example 2 (local):  go {0}".format(example_time.astimezone(None).isoformat(sep=" ", timespec=haptics.TIME_PRECISION_ISO)))
        # 2022-05-28 03:28:57.104000+00:00
        self.__print_notice(" Example 3 (UTC):    go {0}".format(example_time.isoformat(sep=" ", timespec=haptics.TIME_PRECISION_ISO)))

    def __print_playback_status(self):
        status = "paused:" if self.__haptics_player.paused else "playing:"
        self.__print_notice("{0:<8} {1}".format(status, self.__haptics_player.position_formatted))

    def __print_recording_info(self):
        self.__print_notice("Time of recording (local): {0}".format(self.__haptics_player.recording_start.astimezone(None).strftime("%Y-%m-%d %I:%M:%S %p %Z")))
    ...
    def __go_to_timestamp(self, timestamp_str):
        ...
        try:
            ...
            time_delta = (absolute_time - self.__haptics_player.recording_start).total_seconds()
            print(time_delta)
    ...
    def __seek_delta(self, seek_mode, time_delta, delta_type = DeltaType.SECONDS):
        ...
        if seek_mode == self.SeekType.EXACT:
            ...
            self.__haptics_player.position = max(0, self.__haptics_player.position + time_delta)
    ...
    async def console_loop(self):
        ...
        if not self.__haptics_player:
            raise RuntimeError("Haptics player object was not specified!  set_player() must be called with a valid PlayerManager before awaiting console_loop.")
