...
class FeedbackMapperPlayback(FeedbackMapperAbstract):
    def __init__(self, recording_path, on_remark_processed_cb, on_media_processed_cb):
        ...
        self.__haptics_player = recording.HapticsReader(
            recording_path, self.__on_playback_finished
        )
        self.__player_is_seeking = False
        self.__player_last_retrieve_inputs = datetime.datetime.now(
    ...
    def position_formatted(self):
        ...
        return "{0:<13} = {1:<15} [time: {2}]".format(
            str(round(self.position, 6)) + "s",
            str(relative_time),
            absolute_time.isoformat(sep=" ", timespec=haptics.TIME_PRECISION_ISO),
        )
    ...
    def retrieve_inputs(self):
        ...
        if self.__player_is_seeking:
            ...
            time_delta_sec += (
                ...
                - self.__player_last_retrieve_inputs
            ).total_seconds()
        ...
        for record in records:
            ...
            if isinstance(record, recording.EntryInputs):
                ...
                if (
                    not input_peaks
                    or len(record.inputs) != self.__player_last_input_count
                ):
...
class FeedbackMapperAnalogTrigger(FeedbackMapperAbstract):
    ...
    def __init__(self, haptics_input, order):
        ...
        self.__trigger_codes[self.Trigger.LEFT] = [
            evdev.ecodes.ABS_Z,
            evdev.ecodes.ABS_BRAKE,
        ]
        self.__trigger_codes[self.Trigger.RIGHT] = [
            evdev.ecodes.ABS_RZ,
    ...
    def __update_peaks(self):
        for trigger_type in self.Trigger:
            self.__trigger_peak[trigger_type] = max(
                self.__trigger_value[trigger_type], self.__trigger_peak[trigger_type]
            )
    ...
    async def input_queue_loop(self):
        ...
        async for event in self._source.device_events_loop():
            ...
            if (
                self.DEBUG_TRIGGER_DETECT
                and event.value == evdev_driver.AbsInputDevice.TRIGGER_MAX
            ):
                print(
                    "Device: {0}, event: {1} [{2}]".format(
                        self._source.name, evdev.ecodes.ABS[event.code], str(event)
                    )
                )
    ...
    def retrieve_inputs(self):
        ...
        """
        result = [
            self.__trigger_peak[self.Trigger.LEFT]
            / evdev_driver.AbsInputDevice.TRIGGER_MAX,
            self.__trigger_peak[self.Trigger.RIGHT]
            / evdev_driver.AbsInputDevice.TRIGGER_MAX,
        ]
        self.__reset_peaks()
        return result
...
class FeedbackMapperAnalogPedal(FeedbackMapperAbstract):
    ...
    def __init__(self, haptics_input, order):
        ...
        self.__axis_codes[self.Pedal.BRAKE] = [
            evdev.ecodes.ABS_Y,
            evdev.ecodes.ABS_BRAKE,
        ]
    ...
    def __update_peaks(self):
        for trigger_type in self.Pedal:
            self.__axis_peak[trigger_type] = max(
                self.__axis_value[trigger_type], self.__axis_peak[trigger_type]
            )
    ...
    async def input_queue_loop(self):
        ...
        async for event in self._source.device_events_loop():
            ...
            if (
                self.DEBUG_TRIGGER_DETECT
                and event.value == evdev_driver.AbsInputDevice.AXIS_MAX
            ):
                print(
                    "Device: {0}, event: {1} [{2}]".format(
                        self._source.name, evdev.ecodes.ABS[event.code], str(event)
    ...
    def retrieve_inputs(self):
        ...
        # Brake + clutch + gas
        result = [
            self.__axis_peak[self.Pedal.BRAKE] / axis_max,
            self.__axis_peak[self.Pedal.CLUTCH] / axis_max,
            self.__axis_peak[self.Pedal.GAS] / axis_max,
        ]
        self.__reset_peaks()
        return result
...
class SenderManager:
    ...
    def write_sample_config(self):
        ...
        config.set(
            ...
            + ", ".join(
                (
                    self.Config.MODE_IGNORED,
                    self.Config.MODE_ANALOG_TRIGGERS,
                    self.Config.MODE_ANALOG_PEDALS,
                )
            )
            + ")",
        )
        config.set(
            self.Config.HELP,
            "# "
            + self.Config.OPT_ORDER
            + " = relative order in haptics output, starting from 0",
        )
        index = 0
        ...
        self.__logger.debug(
            "Configuration file written to '{0}'".format(self.__config_file_path)
        )
    ...
    def load_config(self):
        ...
        for abs_input_device in abs_input_devices:
            ...
            if filtered_device_id in config:
                ...
                if mode == self.Config.MODE_IGNORED:
                    self.__logger.debug(
                        "Ignoring device '{0}' as requested".format(filtered_device_id)
                    )
                elif mode == self.Config.MODE_ANALOG_TRIGGERS:
                    self.__logger.debug(
                        "Mapping device '{0}' as analog triggers, ordered as '{1}'".format(
                            filtered_device_id, order
                        )
                    )
                    self.__feedback_mappers.append(
                        FeedbackMapperAnalogTrigger(abs_input_device, order)
                    )
                elif mode == self.Config.MODE_ANALOG_PEDALS:
                    self.__logger.debug(
                        "Mapping device '{0}' as analog pedals, ordered as '{1}'".format(
                            filtered_device_id, order
                        )
                    )
                    self.__feedback_mappers.append(
                        FeedbackMapperAnalogPedal(abs_input_device, order)
                    )
                else:
                    self.__logger.debug(
                        "Invalid mode '{1}' for device '{0}'".format(
                            filtered_device_id, mode
                        )
                    )
                    return False
            else:
                self.__logger.debug(
                    "Ignoring unconfigured device '{0}' ({1})".format(
                        filtered_device_id, abs_input_device.name
                    )
                )

        if not len(self.__feedback_mappers):
            self.__logger.warning(
                "No absolute input devices configured, check configuration file '{0}' or write a new one with '--write'".format(
        ...
        self.__logger.debug(
            "Configuration file read from '{0}'".format(self.__config_file_path)
        )
        return True
    ...
    async def input_queue_loop(self):
        ...
        try:
            await asyncio.wait(
                [feedback.input_queue_loop() for feedback in self.__feedback_mappers]
            )
...
class PlayerManager:
    def __init__(
        ...
        self.__feedback_mapper = FeedbackMapperPlayback(
            self.__recording_file,
            self.__on_remark_processed_cb,
            self.__on_media_processed_cb,
        )
    ...
    async def input_queue_loop(self):
        ...
        self.__logger.debug(
            "Playing from recording '{0}'".format(self.__recording_file)
        )
        canceled = False
        try:
    ...
    def __on_media_processed_cb(self, media_command):
        if not media_command.id in self.__media_players:
            ...
            self.__media_players[media_command.id] = MediaPlayer(
                self.__recording_base_dir,
                self.__external_paused_set_cb,
                self.__external_position_set_cb,
                self.__external_quit_cb,
            )
        ...
        if isinstance(media_command, recording.EntryMediaStop):
            ...
            self.__media_players[media_command.id].play_with_offset(
                media_command.time_delta, media_command.offset, media_command.media_file
            )
        else:
            raise Exception(
                "Unexpected/invalid media command: {0}".format(media_command)
            )
    ...
    def __sync_media_timer_cb(self):
        ...
        # Schedule again
        self.__sync_media_timer = asyncio.get_event_loop().call_later(
            haptics.MEDIA_SYNC_RATE_SECS, self.__sync_media_timer_cb
        )
