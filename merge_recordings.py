...
def main(output_file, recording_filenames, include_backups):
    if not include_backups:
        ...
        if backup_files:
            print(
                "Excluding backup files (use '--include-backups' to override): '{0}'".format(
                    "', '".join(backup_files)
                )
            )
            # Exclude all files ending in "~"
            recording_filenames = [
                file for file in recording_filenames if not file.endswith("~")
            ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "output_file",
        help="Output of merged RemoteHaptics session recording",
        metavar="<merged.rec>",
    )
    parser.add_argument(
        "recording_files",
        help="RemoteHaptics session recording to merge",
        nargs="+",
        metavar="<recording.rec>",
    )
    parser.add_argument(
        "-f",
        "--force",
        help="Overwrite the output file when it exists",
        action="store_true",
    )
    parser.add_argument(
        "-i",
        "--include-backups",
        help="Don't exclude backup files (files ending in '~')",
        action="store_true",
    )
    ...
    if os.path.isfile(args.output_file) and not args.force:
        print(
            "Error: Output file '{0}' already exists.  Specify '--force' to overwrite.".format(
                args.output_file
            )
        )
        raise SystemExit
